...
- [x] Auto-sync — new Tally data flows into ARTHA automatically (`tallySyncScheduler`
      every 15 min, incremental voucher window + full parties/outstanding refresh;
      `POST /sync/now` + `GET /sync/status`; frontend "Tally Connect" page `/tally`
      auto-refreshes every 30s)
- [x] Phase 4 — tests: **26 passing** (14 connector + 3 SETU boundary + 8 scheduler)
      incl. read-only proof, credential-leak check, and the SetuDispatch
      field-mapping regression tests
- [x] Core fix — pre-existing `SetuDispatch` snake_case→camelCase ValidationError fixed
      additively in `signalEngine.service.js` + `setuDispatch.service.js` (model
      boundary maps to schema; `setuDispatchBoundary.test.js` proves no snake_case keys
      reach the model). This unblocks the ARTHA signal pipeline: `artha: dispatched`.
...
| Tests | `backend/tests/tallyConnector.test.js` (14) + `setuDispatchBoundary.test.js` (3) + `tallySyncScheduler.test.js` (8) |
| Core fix (SETU boundary) | `backend/src/services/signalEngine.service.js`, `setuDispatch.service.js` |
| Auto-sync | `backend/src/services/tallySyncScheduler.service.js` (+ `/sync/now`, `/sync/status`) |
| Frontend | `frontend/src/pages/tally/TallyConnect.jsx` (`/tally`, auto-refresh 30s) |
| Demo runner | `backend/scripts/tally-connect-demo.js` |
| API | `backend/src/routes/tallyConnector.routes.js` (`/api/v1/tally-connect`) |
| Env config | `backend/.env.example` (`TALLY_*`, `TALLY_SETU_*`, `TALLY_SYNC_*`) |
